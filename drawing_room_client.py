    ...
    def refresh(self):
        ...
        for n, color in enumerate(defined_colors):
            if self.draw_color == defined_colors[color]:
                pygame.draw.rect(self.window, defined_colors['white'], pygame.Rect((300-2 + n * 24, 20-2), (20+4, 20+4)))
        ...
        self.window.blit(self.help_surface, (self.help_surface_rect.x, self.help_surface_rect.y), pygame.Rect(0, 0, self.help_surface_width, self.help_surface_height))
    ...
    def handle_events(self):
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if event.key == pygame.K_z and (keys[pygame.K_LCTRL] or keys[pygame.K_RCTRL]) and not self.chat_selected:
                    self.polygon = []
                ...
                if self.chat_selected == True:
                    if event.key == pygame.K_BACKSPACE:
                        ...
                        if len(self.inp_text2) < 1:
                            self.inp_text2 = ''
                        #send_chat(self.inp_text, self.inp_text2)
                        self.inp_text = ''
                        self.inp_text2 = ''
                    else:
